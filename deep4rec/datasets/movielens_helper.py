...
# ==============================================================================
"""Helper functions to download and extract the MovieLens dataset from GroupLens website.
...
_BUFFER_SIZE = {
    ML_1M: {"train": 107978119, "eval": 26994538},
    ML_20M: {"train": 2175203810, "eval": 543802008},
}
...
NUM_USER_IDS = {ML_1M: 6040, ML_20M: 138493}
...
def _download_and_clean(dataset, data_dir):
  ...
  """
    ...
    if set(expected_files).intersection(tf.io.gfile.listdir(data_subdir)) == set(
        expected_files
    ):
...
def _transform_csv(input_path, output_path, names, skip_first, separator=","):
  ...
  """
    ...
    with tf.io.gfile.GFile(output_path, "wb") as f_out, tf.io.gfile.GFile(
        input_path, "rb"
    ) as f_in:
...
def _check_buffers(data_dir, dataset):
    train_path = os.path.join(
        data_dir, _BUFFER_SUBDIR, "{}_{}_buffer".format(dataset, "train")
    )
    eval_path = os.path.join(
        data_dir, _BUFFER_SUBDIR, "{}_{}_buffer".format(dataset, "eval")
    )
...
def construct_subdatasets(dataset, data_dir):
    ...
    if _check_buffers(data_dir, dataset):
        ...
        df = csv_to_joint_dataframe(dataset=dataset, data_dir=data_dir)
        df = integerize_genres(dataframe=df)
        df = df.drop(columns=[TITLE_COLUMN])
        ...
        return _preprocess_df(train_df), _preprocess_df(eval_df)
